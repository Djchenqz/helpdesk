...
Route::group(['middleware' => ['web']], function () {
    Route::group(['middleware' => ['update', 'install']], function () {
        Auth::routes();
    ...
    /*
      ...
      |
     */
    Route::group(['middleware' => ['roles', 'auth', 'install', 'update']], function () {
        // taiyue not show register page
        // register page
    ...
    /*
      ...
      |
     */
    Route::group(['middleware' => ['auth', 'install', 'update']], function () {
        Route::get('/ticket/user/close/{id}', ['as' => 'ticket.user.close', 'uses' => 'Agent\helpdesk\TicketController@userClose']); /*  Get Ticket Close */
    ...
    Route::group(['middleware' => ['role.agent', 'auth', 'install', 'update']], function () {
...
//testing ckeditor
    ...
    Route::group(['middleware' => ['role.user', 'auth']], function () {
        ...
        Route::get('myorgatickets', ['as' => 'ticket3', 'uses' => 'Client\helpdesk\GuestController@getMyOrgaticket']);
...
//    Route::get('/reportdata', 'HomeController@pushdata');
    ...
    \Event::listen('ticket-assignment', function ($data) {
        $cmd = 'curl "127.0.0.1:8000/awscase/trigger?type=ticket-assignment&id=' . $data['id'] . '"';
        logger()->warning($cmd);
        // $output = shell_exec($cmd);
    });

    \Event::listen('change-status', function ($data) {
        if ($data['status'] == 'Resolved') {
            $cmd = 'curl "127.0.0.1:8000/awscase/trigger?type=resolve&id=' . $data['id'] . '"';
            logger()->warning($cmd);
            // $output = shell_exec($cmd);
        }
    });

    \Event::listen('Reply-Ticket', function ($data) {
        $content = urlencode(base64_encode($data['body']));
        $cmd = 'curl "127.0.0.1:8000/awscase/trigger?type=reply&id=' . $data['ticket_id'] . '&c=' . $content . '"';
        logger()->warning($cmd);
        // $output = shell_exec($cmd);
    });
