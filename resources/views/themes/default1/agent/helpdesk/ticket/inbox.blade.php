...
if (Auth::user()->role == 'agent') {
    $teams = [];
    $atas = App\Model\helpdesk\Agent\Assign_team_agent::where('agent_id', '=', Auth::user()->id)->get();
    foreach ($atas as $ata) {
        $teams[] = $ata->team_id;
    }
    $tickets = App\Model\helpdesk\Ticket\Tickets::whereIn('status', array(1, 7))->whereIn('dept_id', $teams)->orderBy('id', 'DESC')->count();
} else {
