...
class FilterController extends Controller
{
    ...
    public function userIsAgent($table)
    {
        if (Auth::user()->role == 'agent') {
            ...
            $table = $table->where(function ($query) use ($teams) {
                $query->whereIn('tickets.dept_id', $teams)
    ...
    public function departmentFilter($value, $table, $is_mytickets)
    {
        if (count($value) == 1 && (strcasecmp($value[0], 'all') == 0)) {
            ...
            $departmentTickets = $this->userCanSeeTeamsTicket($value);
            if ($departmentTickets[0]) {
                $table = $table->leftJoin('teams as dep', 'tickets.dept_id', '=', 'dep.id')
    ...
    /**
     * @category function to return team ids and access right of teams for agents
     *
     * @param array $teams
     *
     * @var array $requested_team
     *
     * @return array of boolean and array values
     */
    public function userCanSeeTeamsTicket($teams)
    {
        $requested_team = Teams::whereIn('name', $teams)->pluck('id')->toArray();
        if (Auth::user()->role == 'admin') {
            return [true, $requested_team];
        } else {
            $agent_team = [];
            $atas = Assign_team_agent::where('agent_id', '=', Auth::user()->id)->get();
            foreach ($atas as $ata) {
                $agent_team[] = $ata->team_id;
            }
            if (count($requested_team) > 0 && count($agent_team) > 0) {
                return [count(array_intersect($requested_team, $agent_team)) == count($requested_team), $requested_team];
            }
