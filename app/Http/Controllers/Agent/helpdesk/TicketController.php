...
class TicketController extends Controller
{
    ...
    public function post_newticket(CreateTicketRequest $request, CountryCode $code, $api = false)
    {
        try {
            ...
            if ($result[1]) {
                ...
                if ($status == 1) {
                    if ($api != false) {
                        $ticket = Tickets::where('ticket_number', '=', 'AAAA-0000-0000')->select('id')->first();
    ...
    public function thread($id)
    {
        if (Auth::user()->role == 'agent') {
            ...
            $teams = [];
            $atas = Assign_team_agent::where('agent_id', '=', Auth::user()->id)->get();
            foreach ($atas as $ata) {
                $teams[] = $ata->team_id;
            }
            if (in_array($tickets->dept_id, $teams)) {
                $tickets = $tickets;
        ...
        try {
            ...
            if ($system_reply == true) {
                if ($tickets->assigned_to == 0) {
                    ...
                    $user_orga = User_org::where('user_id', '=', $tickets->user_id)->first();
                    if($user_orga != null){
                        $orga_id = $user_orga->org_id;
                        $organization = Organization::where('id', '=', $orga_id)->first();
                        if ($organization != null && $organization->internal_notes != ''){
                            $pmname = json_decode(str_replace('<br>', '', $organization->internal_notes))->PM;
                            if ($pmname != Auth::user()->user_name) {
                                $pmuser = User::where('user_name', '=', $pmname)->first();
                                //mail content
                                $ticket_link = route('ticket.thread', $ticket_id);
                                $master = Auth::user()->first_name.' '.Auth::user()->last_name;

                                try {
                                    $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('0', $tickets->dept_id), $to = ['name' => $pmuser->first_name, 'email' => $pmuser->email], $message = ['subject' => $eventthread->title.'[#'.$tickets->ticket_number.']', 'scenario' => 'assign-ticket-pm'], $template_variables = ['ticket_agent_name' => $pmuser->first_name, 'ticket_number' => $tickets->ticket_number, 'ticket_assigner' => $master, 'ticket_link' => $ticket_link]);
                                } catch (\Exception $e) {
                                    return 0;
                                }
                            }
...
//                Event fire for new ticket['']
            ...
            /// UPB auto send mail to tech leader if ticket is tech support
            /// taiyue tech=3, business tech=4, enterprise tech=5
            if(in_array($helptopic, [3, 4, 5])){
                $user_orga = User_org::where('user_id', '=', $user_id)->first();
                if ($user_orga != null){
                    $organization = Organization::where('id', '=', $user_orga->org_id)->first();
                    if ($organization != null){
                        $inotes = $organization->internal_notes;
                        if ($inotes != ''){
                            $inotes = str_replace('<br>', '', $inotes);
                            $opro = json_decode($inotes);
                            $tleader = $opro->PM;
                            $tleaderObj = User::where('user_name', '=', $tleader)->first();
                            if ($tleaderObj != null){
                                array_push($set_mails, ['to_email' => $tleaderObj->email, 'to_user' => $tleaderObj->first_name, 'to_user_name' => $tleaderObj->first_name]);
                            }
                        }
                    }
                }
            ...
            $link = url('thread/'.strval($threaddata->ticket_id));
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status)
    {
        ...
        $ticket = new Tickets();
        $ticket->ticket_number = $this->ticketNumber($ticket_number); // UPCASE-0000-0041
        $ticket->user_id = $user_id;                                  // 1
        $ticket->dept_id = $dept;                                     // 7
        $ticket->help_topic_id = $helptopic;                          // 1
        $ticket->sla = $sla;                                          // 15
        $ticket->assigned_to = $assignto;                             // null

        $ticket->priority_id = $priority;                             //  1
        $ticket->source = $source;                                    // 3
...
//        dd($ticket);
        ...
        $new_date = date_add($ovdate, date_interval_create_from_date_string($sla_plan->grace_period.' minutes'));
     ...
     * @return type string
     */
    public function userClose($id)
    {
        $ticket = new Tickets();
        return TicketController::close($id, $ticket);
    }
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
    ...
    public function assign($id)
    {
        ...
        foreach ($ticket_array as $id) {
            $ticket = Tickets::where('id', '=', $id)->first();
            $haveAssigned = $ticket->assigned_to;
            if ($assign_to[0] == 'team') {
                ...
                if ($haveAssigned == null || $haveAssigned == 0) {
                    $user_orga = User_org::where('user_id', '=', $ticket->user_id)->first();
                    if($user_orga != null){
                        $orga_id = $user_orga->org_id;
                        $organization = Organization::where('id', '=', $orga_id)->first();
                        if ($organization != null && $organization->internal_notes != ''){
                            $pmname = json_decode(str_replace('<br>', '', $organization->internal_notes))->PM;
                            if ($pmname != $user_detail->user_name) {
                                $pmuser = User::where('user_name', '=', $pmname)->first();

                                try {
                                    $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('0', $ticket->dept_id), $to = ['name' => $pmuser->first_name, 'email' => $pmuser->email], $message = ['subject' => $ticket_subject.'[#'.$ticket_number.']', 'scenario' => 'assign-ticket-pm'], $template_variables = ['ticket_agent_name' => $pmuser->first_name, 'ticket_number' => $ticket_number, 'ticket_assigner' => $master, 'ticket_link' => $ticket_link]);
                                } catch (\Exception $e) {
                                    return 0;
                                }
                            }
    ...
    public static function genreateTableJson($tickets)
    {
        return \Datatables::of($tickets)
                        ...
                        ->editColumn('c_uname', function ($tickets) {
                            ...
                            $user_org = User_org::where('user_id', '=', $tickets->c_uid)->first();
                            if ($user_org != null){
