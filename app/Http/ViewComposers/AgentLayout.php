...
class AgentLayout
{
    ...
    public function departments()
    {
        ...
        if (\Auth::user()->role == 'agent') {
            $tickets = $tickets->whereIn('tickets.dept_id', $this->teams);
        }
        $tickets = $tickets
                ->leftJoin('teams as dep', 'tickets.dept_id', '=', 'dep.id')
    ...
    public function deleted()
    {
        ...
        if ($this->auth->role == 'admin') {
            ...
            return $ticket->where('status', '5')->whereIn('dept_id', $this->teams)
                    ->select('id');
        }
    }
    ...
    public function inbox()
    {
        $table = $this->tickets();
        if (Auth::user()->role == 'agent') {
            $table = $table->whereIn('tickets.dept_id', $this->teams)->orWhere('assigned_to', '=', Auth::user()->id);
        }
